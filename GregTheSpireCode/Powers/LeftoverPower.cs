...
public class LeftoverPower() : GregTheSpirePower
{
    ...
    {
        if (!card.Keywords.Contains(GregTheSpireKeywords.Snack) || card.Owner.Creature != this.Owner || oldPileType == StashCardPile.StashPileType || card.Pile.Type != PileType.Play)
            return;
        for (int i = 0; i < this.Amount; ++i)
        {
            ...
            CardCmd.PreviewCardPileAdd(
                await CardPileCmd.AddGeneratedCardToCombat(card.CreateClone(), StashCardPile.StashPileType,
                    card.Owner), 0.2f);
        }
    }
    ...
    public override async Task AfterCardPlayed(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        if (!cardPlay.Card.Keywords.Contains(GregTheSpireKeywords.Snack) || cardPlay.Card.Owner.Creature != this.Owner || cardPlay.Card.)
            return;
        for (int i = 0; i < this.Amount; ++i)
        {
            this.Flash();
